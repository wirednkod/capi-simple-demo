...
window.onload = () => {
  ...
  void (async () => {
    try {
      ...
      ui.log(`${emojis.brick} Reading at block #${parseInt((currentBlock as Block).block.block.header.number,0)}`);
      ...
      ui.log(`${emojis.stethoscope} Chain is syncing with ${(health as SystemHealth).peers} peers`)
