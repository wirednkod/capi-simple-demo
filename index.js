...
window.onload = () => {
    ...
    void (async () => {
        try {
            const { block: { block } } = await C.readBlock(C.westend).run();
            ui.showSynced();
            ui.log(`${emojis.seedling} Chain is ready (powered by CAPI) - Reading at block #${parseInt(block.header.number, 0)}`, true);
            ...
            ui.log(`${emojis.info} Connected to ${chainName} with ${health.peers} peers.`);
